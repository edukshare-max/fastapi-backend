...
@app.post("/auth/register-first-admin", response_model=UserResponse, tags=["Autenticación"])
async def register_first_admin(user: UserCreate):
    """
    ⚠️ ENDPOINT TEMPORAL - Crear el primer admin sin autenticación.
    Eliminar después de crear el admin inicial.
    """
    try:
        ...
        # Retornar sin contraseña (sin auditoría porque no hay admin aún)
        return UserResponse(**{k: v for k, v in user_dict.items() if k != "password_hash"})
    
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error al crear primer admin: {str(e)}")

@app.post("/auth/register-protected", response_model=UserResponse, tags=["Autenticación"])
async def register_user_protected(user: UserCreate, current_user: UserResponse = Depends(require_role(UserRole.ADMIN))):
    """
    Registrar un nuevo usuario en el sistema.
    Solo accesible para administradores.
    """
